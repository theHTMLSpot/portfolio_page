...
export default function InputWithHoverLabel({ ...props }: Readonly<Input>) {
    ...
    return (
        <div
            className="relative w-full flex flex-col gap-1 pt-5"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
        >
            <label
                htmlFor={name}
                className={`absolute text-sm font-semibold  transition-all duration-200 pointer-events-none ${
                    isHovered || value !== "" ? "top-2.5 left-2 bg-[#0a0a0a] text-white" : "top-[50%] left-2 text-gray-400"
                }`}
            >
                {label}
            </label>
